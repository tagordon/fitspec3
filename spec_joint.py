...
def run(
    ...
    def run_single_band(ind):
        ...
        for dv, dc, mask in zip(detrending_vectors, detrending_cubes, masks):
            if dc is not None:
                if dv is not None:
                    new_detrending_vectors.append(np.hstack([dv[mask], dc[ind][mask]]))
                else:
                    new_detrending_vectors.append(dc[ind][mask])
            else: 
                if dv is not None:
